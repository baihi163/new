...
### 2.3 与任务三的改动对比
相比于任务三的模型和训练方法，我在任务四中进行了以下关键改动，以提升 CIFAR-10 数据集上的分类性能：

- **模型结构改动**：
  - 任务三使用的是一个自定义的简单 CNN 模型（`SimpleCNN`），包含 4 层卷积和 3 层全连接层，结构较为基础，未使用残差连接或注意力机制。
  - 任务四升级为基于 ResNet18 的改进模型（`SEResNet`），并在每个 `BasicBlock` 中嵌入了 Squeeze-and-Excitation (SE) 模块，通过学习通道权重增强了特征表达能力。这一改动使模型能够聚焦于更重要的特征通道。
- **训练策略改动**：
  - 任务三仅使用基本的数据增强方法（如 `RandomCrop` 和 `RandomHorizontalFlip`）以及 `StepLR` 学习率调度。
  - 任务四引入了 Mixup 数据增强策略（`alpha=1.0`），通过线性插值混合样本和标签，增强了模型的正则化效果和泛化能力；同时采用 `CosineAnnealingLR` 学习率调度，使学习率在训练后期平滑衰减，帮助模型更好地收敛。
- **性能结果对比**：
  - 任务三的测试集准确率较低（具体数值未记录），未达到任务四的性能水平。
  - 任务四通过上述改动，将准确率提升至 **94.55%**，显著优于任务三的表现，同时保持了模型轻量化（参数量 11.26M，FLOPs 558.22M）。

通过这些改动，我在保持模型轻量化的同时，大幅提升了分类性能，验证了结构优化和策略优化的有效性。
